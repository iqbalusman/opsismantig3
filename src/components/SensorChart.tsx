...
const CardShell: React.FC<{ title?: string; children: React.ReactNode; className?: string }>=({ title, children, className }) => (
  ...
  >
    {/* Background gradient ala HealthStatus (blue → sky) */}
    <div
      aria-hidden
    ...
    {title && (
      <div className="mb-4 flex items-center gap-2 text-slate-900">
        ...
        <span className="ml-auto text-[10px] px-2 py-1 rounded-full uppercase tracking-wide bg-white/70 text-slate-600 border border-white/60">Live</span>
      </div>
    )}
...
// ===== Panel (inner small cards) =====
const Panel: React.FC<{ title: string; height: number; icon?: React.ReactNode; gradient?: string; children: React.ReactNode }>=({ title, height, icon, gradient = "from-blue-500 to-cyan-500", children }) => (
  <motion.div
...
// Tooltip kustom agar selaras tema
const CustomTooltip: React.FC<{ active?: boolean; label?: any; payload?: any[]; timeFormatter: (v: any) => string }>=({ active, label, payload, timeFormatter }) => {
...
const SensorChart: React.FC<SensorChartProps> = ({ data, height = 320, timeZone, className }) => {
  ...
  return (
    <CardShell title="Grafik Sensor" className={className}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {safe.length === 0 ? (
            ...
            <ResponsiveContainer>
              <LineChart data={safe} margin={{ top: 8, right: 24, left: 12, bottom: 8 }}>
                ...
                <Line type="monotone" dataKey="suhu_ikan" name="Suhu" stroke="url(#gradSuhu)" strokeWidth={3} dot={false} activeDot={{ r: 5 }} connectNulls />
                {latest?.suhu_ikan != null && (
                  <ReferenceDot x={latest.timestamp as any} y={latest.suhu_ikan as any} r={4} fill="#06b6d4" stroke="#fff" strokeWidth={2} />
                )}
              </LineChart>
            </ResponsiveContainer>
          )}
          ...
          {safe.length === 0 ? (
            ...
            <ResponsiveContainer>
              <LineChart data={safe} margin={{ top: 8, right: 24, left: 12, bottom: 8 }}>
                ...
                <YAxis domain={["auto","auto"]} stroke="#64748b" fontSize={12}/>
                <Tooltip content={<CustomTooltip timeFormatter={formatX} />} />
                <Line type="monotone" dataKey="avg_rgb" name="AVG RGB" stroke="url(#gradRgb)" strokeWidth={3} dot={false} activeDot={{ r: 5 }} connectNulls />
                {latest?.avg_rgb != null && (
                  <ReferenceDot x={latest.timestamp as any} y={latest.avg_rgb as any} r={4} fill="#0ea5e9" stroke="#fff" strokeWidth={2} />
                )}
              </LineChart>
            </ResponsiveContainer>
          )}
        ...
        {/* Nilai Gas */}
        <Panel title="Grafik Nilai Gas" height={height} icon={<Wind className="w-4 h-4" />} gradient="from-cyan-500 to-blue-700">
          {safe.length === 0 ? (
            <div className="h-full grid place-items-center text-sm text-slate-600">Belum ada data.</div>
          ) : (
            <ResponsiveContainer>
              <LineChart data={safe} margin={{ top: 8, right: 24, left: 12, bottom: 8 }}>
                ...
                <YAxis domain={["auto","auto"]} stroke="#64748b" fontSize={12}/>
                <Tooltip content={<CustomTooltip timeFormatter={formatX} />} />
                <Line type="monotone" dataKey="nilai_gas" name="Nilai Gas" stroke="url(#gradGas)" strokeWidth={3} dot={false} activeDot={{ r: 5 }} connectNulls />
                {latest?.nilai_gas != null && (
                  <ReferenceDot x={latest.timestamp as any} y={latest.nilai_gas as any} r={4} fill="#1d4ed8" stroke="#fff" strokeWidth={2} />
                )}
              </LineChart>
            </ResponsiveContainer>
          )}
