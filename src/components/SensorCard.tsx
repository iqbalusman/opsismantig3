...
const formatNum = (n: number | undefined | null, unit = "") => {
  ...
  // kalau bilangan bulat, tampilkan tanpa .00
  if (Number.isInteger(num)) {
    return `${num}${unit}`;
  }

  // kalau ada desimal, batasi 2 angka di belakang koma
...
const IconWrap: React.FC<{ gradientFrom?: string; gradientTo?: string; children: React.ReactNode }> = ({
  ...
  <div
    className={`p-3 rounded-2xl shadow-lg bg-gradient-to-br ${gradientFrom} ${gradientTo} text-white ring-1 ring-white/30`}
  >
    {children}
  </div>
);
...
const StatCard: React.FC<{
  ...
  >
    ...
    <div className="flex items-start justify-between gap-4">
      ...
      <span className="text-[10px] uppercase tracking-wide px-2 py-1 rounded-full bg-white/70 dark:bg-slate-800/70 text-slate-600 dark:text-slate-300 border border-slate-200/60 dark:border-slate-700/60">
        Live
      </span>
    </div>
    <div className="mt-4 text-3xl font-extrabold leading-none text-slate-900 dark:text-white">{value}</div>
    {footerRight && (
...
const Pill: React.FC<{ label: string; intent?: "good" | "warn" | "bad" | "neutral" }> = ({
  label,
  intent = "neutral",
}) => {
  const map = {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] border ${map[intent]}`}
    >
...
const SensorCard: React.FC<SensorCardProps> = ({ row, className }) => {
  ...
  const rgb = formatNum(row.avg_rgb);
  const gas = formatNum(row.nilai_gas);

  return (
    <div
      className={`relative rounded-3xl p-6 md:p-7 border border-white/50 shadow-2xl overflow-hidden ${
        className ?? ""
      }`}
    >
      {/* Background gradient */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard
          ...
          title="Suhu Ikan"
          value={suhu}
          subtitle="Pemantauan suhu tubuh ikan"
        ...
        <StatCard
          ...
          footerRight={
            <span
              className={`px-2.5 py-1 rounded-full text-[11px] border ${badgeColor(row.status_h)}`}
            >
              {row.status_h || "—"}
            </span>
          }
        ...
        <StatCard
          ...
          value={gas}
          subtitle="Pengukuran kadar gas"
          footerRight={
            <span
              className={`px-2.5 py-1 rounded-full text-[11px] border ${badgeColor(row.status_gas)}`}
            >
              {row.status_gas || "—"}
            </span>
          }
