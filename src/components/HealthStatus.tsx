...
// FINAL: Persentase halus berbasis AVG & GAS (suhu hanya monitoring)
// - AVG: 0–250 = Segar, 250–400 = Kurang Segar, >400 = Tidak Segar
// - GAS: 0–2500 = Segar, 2500–3500 = Kurang Segar, >3500 = Tidak Segar
// - Suhu NETRAL secara default; berubah ke Dingin (≤4 °C) atau Panas (≥30 °C)
// - Persentase dihitung kontinu (piecewise-linear) + soft-min konservatif
...
type Props = {
  avg_rgb?: number | null;   // nilai AVG
  nilai_gas?: number | null; // nilai GAS
  suhu_ikan?: number | null; // °C (monitoring)
  fishName?: string;
  animate?: boolean;
};
...
// Map ke 0% di luar zona "Tidak Segar"
const AVG_CAP_DANGER = 700;
const GAS_CAP_DANGER = 5000;

// Soft-min mixing (0=min; 0.25 = tambah 25% dari gap)
const SOFT_MIN_ALPHA = 0.25;

// SUHU monitoring
const TEMP_COLD_MAX = 4;   // ≤4 °C dianggap "Dingin (ideal)"
const TEMP_HOT_MIN  = 30;  // ≥30 °C dianggap "Panas (tinggi)"; selain itu NETRAL
...
function scoreFromAvg(avg?: number | null): number {
  ...
  if (x <= AVG_GOOD_MAX) {
    const t = clamp01(x / AVG_GOOD_MAX);
    return Math.round(lerp(100, 80, t));
  }
  if (x <= AVG_WARN_MAX) {
    const t = clamp01((x - AVG_GOOD_MAX) / (AVG_WARN_MAX - AVG_GOOD_MAX));
    return Math.round(lerp(80, 40, t));
  }
  const t = clamp01((x - AVG_WARN_MAX) / (AVG_CAP_DANGER - AVG_WARN_MAX));
  return Math.round(lerp(40, 0, t));
}
...
function scoreFromGas(gas?: number | null): number {
  ...
  if (x <= GAS_GOOD_MAX) {
    const t = clamp01(x / GAS_GOOD_MAX);
    return Math.round(lerp(100, 80, t));
  }
  if (x <= GAS_WARN_MAX) {
    const t = clamp01((x - GAS_GOOD_MAX) / (GAS_WARN_MAX - GAS_GOOD_MAX));
    return Math.round(lerp(80, 40, t));
  }
  const t = clamp01((x - GAS_WARN_MAX) / (GAS_CAP_DANGER - GAS_WARN_MAX));
  return Math.round(lerp(40, 0, t));
}

function combineScoresConservative(a: number, b: number, alpha = SOFT_MIN_ALPHA): number {
  const lo = Math.min(a, b);
  const hi = Math.max(a, b);
  return Math.round(lo + alpha * (hi - lo));
}

/* ======================== Klasifikasi label ======================== */
function classifyAvg(avg?: number | null): { label: "Segar" | "Kurang Segar" | "Tidak Segar"; tone: Tone; score: number } {
  ...
  return { label: "Tidak Segar", tone: "danger", score };
}
function classifyGas(gas?: number | null): { label: "Segar" | "Kurang Segar" | "Tidak Segar"; tone: Tone; score: number } {
...
/** Suhu → NETRAL by default; hanya berubah jika dingin/panas */
  ...
  switch (tone) {
    ...
    default: // neutral
      return {
        ...
        iconWrap: "bg-slate-50/80 border-slate-200 ring-slate-200/50",
        icon: "text-slate-600",
...
function StatusBadge({ tone }: { tone: Tone }) {
  ...
  const text =
    tone === "ok" ? "Segar" :
    tone === "warn" ? "Kurang Segar" :
    tone === "danger" ? "Tidak Segar" : "Netral";
  return (
...
const HealthStatus: React.FC<Props> = ({
  ...
  // Suhu NETRAL by default; hanya dingin/panas yang mengubah tampilan
  const suhuDisp = useMemo(() => temperatureDisplay(suhu_ikan), [suhu_ikan]);

  // Persentase akhir (soft-min dari skor AVG & GAS) — suhu tidak mempengaruhi
  const score = useMemo(
    () => combineScoresConservative(avgClass.score, gasClass.score),
    [avgClass.score, gasClass.score]
  );

  // Verdict konsumsi (hanya dari AVG & GAS)
  const anyDanger = avgClass.tone === "danger" || gasClass.tone === "danger";
  const anyWarn   = avgClass.tone === "warn"   || gasClass.tone === "warn";
  const isUnsafe       = anyDanger;
  const reheatOnly     = !isUnsafe && anyWarn;
  const isSafeToEat    = !isUnsafe && !reheatOnly;

  const StatusIcon = isUnsafe ? XCircle : isSafeToEat ? CheckCircle : AlertCircle;
  const consumptionMessage = isUnsafe
    ? `${fishName} TIDAK LAYAK dikonsumsi.`
    : reheatOnly
    ? `${fishName} PERLU DIPANASKAN hingga matang.`
    : `${fishName} AMAN untuk dikonsumsi.`;
  const actionTips = isUnsafe
    ? "Nilai AVG/GAS menunjukkan ketidaksegaran. Buang produk sesuai prosedur."
    : reheatOnly
    ? "Panaskan hingga inti >70°C. Jika aroma menyengat/asam, jangan dikonsumsi."
    : "Simpan pada 0–4°C. Konsumsi 24–48 jam.";

  const progressColor = isUnsafe ? "from-red-600 to-red-800" : reheatOnly ? "from-orange-300 to-orange-500" : "from-green-300 to-green-500";
  const verdictBadge  = isUnsafe ? "bg-red-50 text-red-800 border-red-300" : reheatOnly ? "bg-orange-50 text-orange-800 border-orange-300" : "bg-green-50 text-green-800 border-green-300";

  // Kartu status — label saja
  const cards = [
    { title: "Status AVG", label: avgClass.label, tone: avgClass.tone as Tone, icon: Palette, hint: "0–250 Segar · 250–400 Kurang · >400 Tidak" },
    { title: "Status GAS", label: gasClass.label, tone: gasClass.tone as Tone, icon: Flame,   hint: "0–2500 Segar · 2500–3500 Kurang · >3500 Tidak" },
    { title: "Suhu (monitoring)", label: suhuDisp.label, tone: suhuDisp.tone, icon: Thermometer, hint: "Hanya pemantauan; tidak mempengaruhi persentase" },
  ] as const;
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-sm text-slate-600">Persentase halus dari AVG & GAS (soft-min). Suhu bersifat NETRAL dan hanya berubah saat dingin/panas.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <StatusIcon role="img" aria-label={isUnsafe ? "Tidak Segar" : reheatOnly ? "Kurang Segar" : "Segar"}
                        className={`w-5 h-5 ${isUnsafe ? "text-red-700" : reheatOnly ? "text-orange-600" : "text-green-600"}`} />
      ...
      <div className="w-full bg-white/60 rounded-full h-3 overflow-hidden mb-5 shadow-inner">
        <motion.div initial={animate ? { width: 0 } : false}
                    animate={animate ? { width: `${score}%` } : undefined}
                    transition={{ duration: 1, ease: "easeOut" }}
                    className={`h-full bg-gradient-to-r ${progressColor} rounded-full`}
                    role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={score} />
      </div>
      ...
      {/* Peringatan & saran */}
      <motion.div initial={animate ? { opacity: 0 } : false}
                  animate={animate ? { opacity: 1 } : undefined}
                  transition={{ delay: 0.2 }}
                  className={`mt-6 p-4 rounded-xl border-l-4 ${
                    isUnsafe ? "bg-red-50/90 border-l-red-700 text-red-800"
                             : reheatOnly ? "bg-orange-50/90 border-l-orange-600 text-orange-800"
